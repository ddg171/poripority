...
const categoryName = computed<string>(() => {
  ...
  if (!categoryStore.state.value.length) {
    return "";
  }
  return (
    categoryStore.state.value?.find((c) => c.id === category.value)?.name || ""
  );
});
...
const pageTitle = computed<PageTitleProp>(() => {
  ...
  const subtitle =
    totalCount.value === 0
      ? "全0件中0件を表示中"
      : `全${totalCount.value}件中${offset.value + 1}-${offset.value + articles.value.length}件を表示中`;
  return {
    title,
    subtitles: [subtitle],
    topImg: {
...
const title = computed<string>(() => {
  return (
    (category.value ? `${categoryName.value}の記事一覧` : "記事一覧") +
    "-" +
...
const description = computed<string>(() => {
  return (
    `${categoryName.value ? categoryName.value + "に関する" : "全"}記事一覧/` +
    (totalCount.value === 0
      ? "全0件中0件を表示"
      : `全${totalCount.value}件中${offset.value + 1}-${offset.value + articles.value.length}件目を表示`)
  );
});
useSeoMeta({
  title: () => `${title.value}`,
